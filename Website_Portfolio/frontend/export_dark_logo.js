...
<div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
  ...
  <div style="color: white; font-family: 'Space Grotesk', sans-serif; text-align: center;">
    <div style="font-size: 2.2rem; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase;">Anas Tarayra</div>
    <div style="font-size: 1rem; color: #38BDF8; letter-spacing: 0.1em; text-transform: uppercase; margin-top: 5px;">UI/UX & Engineering</div>
  </div>
</div>
`;
...
(async () => {
  try {
    const tempHtmlPath = path.resolve('temp_dark_logo.html');
    ...
    const outPath = path.join(outDir, 'Logo_Dark_Background.png');
    await page.screenshot({ path: outPath, omitBackground: true });
    
    await browser.close();
    fs.unlinkSync(tempHtmlPath);
    console.log('Dark Logo PNG saved to:', outPath);
  } catch(e) {
    console.error(e);
  }
})();
